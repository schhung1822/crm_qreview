...
function out(r: { id: string; connectionId: string; article: unknown; alternates: unknown; articleId?: string | null; status: string; runAt?: Date | null; attempts: number; maxAttempts: number; lastError?: string | null; resultPostId?: string | null; resultUrl?: string | null; createdAt: Date; updatedAt: Date }): PublishJob { return { id: r.id, connectionId: r.connectionId, article: r.article as PublishArticle, alternates: r.alternates as Array<{ locale: string; url: string }>, articleId: r.articleId ?? undefined, status: r.status as PublishJobStatus, runAt: r.runAt?.toISOString(), attempts: r.attempts, maxAttempts: r.maxAttempts, lastError: r.lastError ?? undefined, resultPostId: r.resultPostId ?? undefined, resultUrl: r.resultUrl ?? undefined, createdAt: r.createdAt.toISOString(), updatedAt: r.updatedAt.toISOString() }; }
async function readAll(): Promise<PublishJob[]> { if (isDb()) return (await prisma.publishJob.findMany({ where: { bizId: biz() } })).map(out); return readJson<PublishJob[]>(bizFile(NAME), []); }
...
export async function updateJob(id: string, patch: Partial<PublishJob>): Promise<void> {
  if (isDb()) { const data: Record<string, unknown> = { updatedAt: new Date() }; for (const k of ['status','attempts','lastError','resultPostId','resultUrl','articleId','maxAttempts'] as const) if (patch[k] !== undefined) data[k] = patch[k]; if (patch.runAt !== undefined) data.runAt = patch.runAt ? new Date(patch.runAt) : null; if (patch.article !== undefined) data.article = patch.article as object; if (patch.alternates !== undefined) data.alternates = patch.alternates as object; await prisma.publishJob.updateMany({ where: { id, bizId: biz() }, data }); return; }
...
export async function deleteJob(id: string): Promise<void> { if (isDb()) { await prisma.publishJob.deleteMany({ where: { id, bizId: biz() } }); return; } await mutateJson<PublishJob[], void>(bizFile(NAME), [], (rows) => [rows.filter((r) => r.id !== id), undefined]); }

...
const PLATFORM_META: Record<SocialChannelKind, { label: string; color: string }> = {
  ...
  taobao: { label: 'Taobao', color: '#ff4400' },
  taobaoshop: { label: 'Taobao', color: '#ff4400' },
};
...
const PLATFORM_LOGO_SVG: Record<
  Exclude<SocialChannelKind, 'shopeeshop' | 'tiktokshop' | 'tiktokshopshop' | 'lazadashop' | 'taobaoshop' | 'fbprofile'>,
  (s: number) => string
> = {
  facebook: (s) =>
  ...
  // Taobao: icon nhận diện dựng bằng SVG - nền cam thương hiệu (#ff4400) bo góc + chữ 淘
  // trắng (chữ Hán "Tao" trong logo app Taobao). Không nhúng logo bitmap để giữ 1 nguồn SVG.
  taobao: (s) =>
    `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" width="${s}" height="${s}" aria-hidden="true"><rect width="48" height="48" fill="#ff4400" rx="10"/><text x="24" y="33.5" font-family="'PingFang SC','Microsoft YaHei','Noto Sans CJK SC',sans-serif" font-weight="700" font-size="26" fill="#ffffff" text-anchor="middle">淘</text></svg>`,
...
function buildSections(r: SocialReportRecord, L: SocialReportLabels): Section[] {
  ...
  if (r.platform === 'ecom') out.push(...ecomChartSections(r, L, locale));
    ...
    if (ch0.kind === 'shopee' || ch0.kind === 'tiktokshop' || ch0.kind === 'lazada' || ch0.kind === 'taobao')
      out.push(...productChartSections(ch0, L, locale));
    else if (ch0.kind === 'shopeeshop' || ch0.kind === 'tiktokshopshop' || ch0.kind === 'lazadashop' || ch0.kind === 'taobaoshop')
  ...
  for (const ch of r.channels) {
    ...
    // Kênh SẢN PHẨM (Shopee/TikTok Shop/Lazada/Taobao): mục riêng, không có bài đăng.
    if (ch.kind === 'shopee' || ch.kind === 'tiktokshop' || ch.kind === 'lazada' || ch.kind === 'taobao') {
      out.push(...shopeeSections(ch, L, locale));
      continue;
    }
    // Kênh SHOP (Shopee/TikTok Shop/Lazada/Taobao): info shop/danh mục/chỉ số/đánh giá theo sản phẩm.
    if (ch.kind === 'shopeeshop' || ch.kind === 'tiktokshopshop' || ch.kind === 'lazadashop' || ch.kind === 'taobaoshop') {
...
function buildHeader(r: SocialReportRecord, L: SocialReportLabels): string {
  ...
  } else if (single?.kind === 'shopee' || single?.kind === 'tiktokshop' || single?.kind === 'lazada' || single?.kind === 'taobao') {
  ...
  } else if (single?.kind === 'shopeeshop' || single?.kind === 'tiktokshopshop' || single?.kind === 'lazadashop' || single?.kind === 'taobaoshop') {

...
export async function POST(req: Request) {
  ...
  const acct = await sharedIsLimited(acctKey, 20); // >=20 lần sai trong 15 phút → tạm khóa tài khoản
  if (acct.limited) {
    return NextResponse.json(
  ...
  if (!user) {
    await sharedRecordFailure(acctKey, 15 * 60 * 1000); // ghi 1 lần thất bại cho tài khoản
    return NextResponse.json({ error: 'Email hoặc mật khẩu không đúng' }, { status: 401 });
  }
  await sharedClearRateLimit(acctKey); // đăng nhập đúng → xóa bộ đếm thất bại của tài khoản

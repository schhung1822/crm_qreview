  ...
  imageProcessing: z
    .object({
      ...
      barHeight: z
        .number()
        .min(0)
        .max(80, 'Độ dày khung phải từ 0 đến 80px')
        .optional(),
...
export async function POST(req: Request) {
  ...
  const connectionIds = Array.from(
    new Set(
      [...(parsed.data.connectionIds ?? []), parsed.data.connectionId].filter(Boolean),
    ),
  ) as string[];
  if (connectionIds.length === 0) {
    return NextResponse.json(
      { error: 'Vui lòng chọn ít nhất một kết nối mạng xã hội' },
      { status: 400 },
    );
  }
  ...
  for (const url of [
    ...
    if (!/^https?:\/\//i.test(url)) {
      return NextResponse.json(
        { error: 'Ảnh, video và liên kết phải dùng URL http(s) công khai' },
        { status: 400 },
      );
    }
    try {
  ...
  const affiliateLinks = (parsed.data.affiliateLinks ?? [])
    .map((link) => link.trim())
    .filter(Boolean);
  if (affiliateLinks.some((link) => !/https?:\/\/\S+/i.test(link))) {
    return NextResponse.json(
      { error: 'Mỗi link affiliate phải chứa một URL http(s)' },
      { status: 400 },
    );
  }
  ...
  if (urlSource && !/^https?:\/\//i.test(urlSource)) {
    return NextResponse.json(
      { error: 'Link bài nguồn phải là URL http(s)' },
      { status: 400 },
    );
  }
  const originalMediaUrls =
  ...
  if (parsed.data.mediaType === 'image') {
    ...
    if (rawImageUrls.length === 0) {
      return NextResponse.json(
        { error: 'Vui lòng nhập ít nhất một URL ảnh' },
        { status: 400 },
      );
    }
    if (parsed.data.imageProcessing?.enabled === false) {
      ...
      try {
        const processedUrls = await processSocialImageUrls(
          rawImageUrls,
          req,
          parsed.data.title || 'social-image',
        ...
        const message =
          error instanceof Error ? error.message : 'Không thể xử lý ảnh trước khi đăng';
  ...
  const loadedConnections = await runWithBiz({ userId: g.user.id, bizId: g.bizId }, () =>
    Promise.all(
      connectionIds.map((id) =>
        getConnectionCreds(id).then((loaded) => ({ id, loaded })),
      ),
    ),
  );
  ...
  if (missing)
    return NextResponse.json(
      { error: `Không tìm thấy kết nối ${missing.id}` },
      { status: 404 },
    );
  const socialConnections = loadedConnections.map((item) => item.loaded!);
  const invalid = socialConnections.find(
    (item) => !isSocialProvider(item.record.provider),
  );
  if (invalid) {
    return NextResponse.json(
      { error: `Kết nối ${invalid.record.label} không phải mạng xã hội` },
      { status: 400 },
    );
  }
    ...
    socialConnections.map(async (loaded) => {
      try {
        const result = await publishSocial(
          loaded.record.provider as SocialProvider,
          loaded.credentials,
          publishInput,
        );
        await runWithBiz({ userId: g.user.id, bizId: g.bizId }, () =>
          setConnectionStatus(loaded.record.id, 'active'),
        );
        recordUserEvent({
        ...
        const rawMessage =
          error instanceof Error ? error.message : 'Không thể đăng nội dung';
        const message = providerErrorMessage(
          loaded.record.provider,
          graphPermissionMessage(loaded.record.provider, rawMessage),
        );
        recordUserEvent({
          eventName: 'social_publish',
          eventType: 'conversion',
  ...
  await runWithBiz({ userId: g.user.id, bizId: g.bizId }, () =>
    ...
    console.error(
      '[social-posts] save history failed:',
      error instanceof Error ? error.message : error,
    );
  });
  if (successCount === 0) {
    return NextResponse.json(
      { error: results[0]?.error || 'Không thể đăng nội dung', results },
      { status: 502 },
    );
  }
  ...
  if (parsed.data.reviewPostIds?.length) {
    await runWithBiz({ userId: g.user.id, bizId: g.bizId }, async () => {
      ...
      console.error(
        '[social-posts] cleanup reviewed drafts failed:',
        error instanceof Error ? error.message : error,
      );
    });
  }
  ...
  return NextResponse.json({
    published:
      failedCount === 0 &&
      results.every((item) => item.ok && item.result?.status === 'published'),
